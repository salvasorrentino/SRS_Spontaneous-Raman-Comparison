...
path_fold = (r'C:\Users\salva\OneDrive - Massachusetts Institute of Technology\Coregistration '
             r'SRS-confocal\data\colon')
path_img_srs = os.path.join(path_fold, "colon_srs_hyperspectral_processed.pickle")
...
path_img_confocal = os.path.join(path_fold, "colon_spontaneous_raman.pickle")
...
from colon_comparison.registration_script import (coregistration_algo, resize_by_factor, rotate_scale_image_keep_all,
                                                 raman_hw)
...
plt.figure()
vmin2 = np.percentile(img_1450_srs_rot, 20)
vmax2 = np.percentile(img_1450_srs_rot, 99.9)
plt.imshow(img_1450_srs_rot[:, :], cmap='Blues', vmin=vmin2, vmax=vmax2)
plt.figure()
vmin1 = np.percentile(img_1450_confocal, 20)
vmax1 = np.percentile(img_1450_confocal, 99.9)
plt.imshow(img_1450_confocal, cmap='Reds', vmin=vmin1, vmax=vmax1, alpha=0.4)
plt.show()
...
plt.figure()
plt.plot(calib_confocal, image_confocal[pos0, pos1, :]/image_confocal[pos0, pos1, :].max())
plt.plot(calib_srs, (srs_rot_best[pos0, pos1, :]/srs_rot_best[pos0, pos1, :].max()) -
         (srs_rot_best[pos0, pos1, :]/srs_rot_best[pos0, pos1, :].max()).min())
plt.show()

confocal_fil = image_confocal.copy()
confocal_fil[~srs_rot_best.any(axis=2)] = 0

pd.to_pickle(confocal_fil, os.path.join(path_fold, "colon_srs_hyperspectral_registered_confocal.pickle"))
