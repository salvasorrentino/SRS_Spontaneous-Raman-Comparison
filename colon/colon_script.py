...
image_use = np.delete(image, 31, axis=0)
image_use = np.delete(image_use, 27, axis=0)
image = image_use.copy()

raman_shifts_use = np.delete(raman_shifts, 31)
raman_shifts_use = np.delete(raman_shifts_use, 27)
raman_shifts = raman_shifts_use.copy()

plt.figure()
plt.plot(raman_shifts, image.T[440:, 250:, :].mean(axis=(0, 1)))
img_backfree1 = image.T -  image.T[440:, 250:, :].mean(axis=(0, 1))[None, None, :]
plt.figure()
plt.plot(raman_shifts, img_backfree1[:, :, :].mean(axis=(0, 1)))
plt.plot(raman_shifts, image.T[:, :, :].mean(axis=(0, 1)))

arr_data_proc = savgol_filter(img_backfree1.T, 7, 3)
...
shifted_basesub_resh = np.array(corr_flat).reshape(468, 289, -1)

shifted_basesub_resh2 = shifted_basesub_resh - shifted_basesub_resh.min(axis=2)[:, :, None]

plt.figure()
plt.plot(raman_shifts, shifted_basesub_resh2[440:, 250:, :].mean(axis=(0, 1)))

img_backfree2 = shifted_basesub_resh2 -  shifted_basesub_resh2[440:, 250:, :].mean(axis=(0, 1))[None, None, :]
plt.figure()
plt.plot(raman_shifts, img_backfree2[:, :, :].mean(axis=(0, 1)))
plt.plot(raman_shifts, shifted_basesub_resh2[:, :, :].mean(axis=(0, 1)))

plt.figure()
plt.plot(img_backfree2[:, :, :].mean(axis=(0, 1)))
...
pd.to_pickle(shifted_basesub_resh2_corr, r'C:\Users\salva\OneDrive - Massachusetts Institute of Technology\Coregistration'
            r' SRS-confocal\data\20260518_colon\colon_srs_hyperspectral_processed.pickle')
