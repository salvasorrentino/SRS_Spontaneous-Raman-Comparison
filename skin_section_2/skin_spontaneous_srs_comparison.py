...
path_fold = (r'C:\Users\salva\OneDrive - Massachusetts Institute of Technology\Coregistration '
             r'SRS-confocal\data\skin_section_2')
path_img_srs = os.path.join(path_fold, "skin_srs_small_sampling_hyperspectral.pickle")
...
img_1450_confocal = image_confocal[:, :, 690:715].sum(axis=2)

plt.figure()
plt.imshow(img_1450_confocal, cmap='inferno', alpha=1)
plt.show()

from skin_small_hyperspectral.registration_script import (coregistration_algo, resize_by_factor, rotate_scale_image_keep_all,
                                                 raman_hw)
...
vmax2 = np.percentile(img_1450_srs_rot, 99.99)
...
pd.to_pickle(srs_rot_best, os.path.join(path_fold, "skin_srs_small_sampling_hyperspectral_registered_confocal.pickle"))
